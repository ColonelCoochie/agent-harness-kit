...
async function withStateLock(operation) {
  ...
  const acquire = async (allowRecovery) => {
    try {
      ...
      const ownerIsGone = Number.isInteger(owner?.pid) && !isProcessAlive(owner.pid);
      if (allowRecovery && (age > 300000 || ownerIsGone)) {
        await rm(lockDir, { recursive: true, force: true });
        return acquire(false);
      }
      const ownerText = Number.isInteger(owner?.pid) ? ` pid=${owner.pid}` : '';
...
async function addFeature() {
  ...
  const criteria = list(args.criterion);
  const acceptanceCommands = list(args.acceptCommand || args.command);
  ...
  state.features.push({
    ...
    scope: {
      ...
      deny: list(args.deny).length ? list(args.deny) : config.scope.defaultDeny
    },
    acceptance,
    evidence: [],
    blocker: null,
...
async function startFeature() {
  ...
  feature.scopeBaseline = await repositoryChanges();
  if (config.scope?.enforcement === 'required' && !feature.scopeBaseline.available) {
    throw new Error('Git-backed scope enforcement is required, but repository state is unavailable. Initialize Git and retry.');
  }
  feature.contract = {
...
async function evaluateScope(feature, config) {
  ...
  const changed = current.files.filter((item) => baseline.get(item.path) !== item.hash);
